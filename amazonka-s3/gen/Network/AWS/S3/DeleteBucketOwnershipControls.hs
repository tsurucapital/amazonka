...
data DeleteBucketOwnershipControls = DeleteBucketOwnershipControls'{_dbocExpectedBucketOwner
                                                                    ::
                                                                    !(Maybe
                                                                        Text),
                                                                    _dbocBucket
...
deleteBucketOwnershipControls
  :: BucketName -- ^ 'dbocBucket'
  -> DeleteBucketOwnershipControls
deleteBucketOwnershipControls pBucket_ = DeleteBucketOwnershipControls'
  { _dbocExpectedBucketOwner = Nothing
...
dbocExpectedBucketOwner =
  lens _dbocExpectedBucketOwner (\s a -> s { _dbocExpectedBucketOwner = a })

-- | The Amazon S3 bucket whose @OwnershipControls@ you want to delete.
dbocBucket :: Lens' DeleteBucketOwnershipControls BucketName
dbocBucket = lens _dbocBucket (\s a -> s { _dbocBucket = a })

instance AWSRequest DeleteBucketOwnershipControls
         where
  type Rs DeleteBucketOwnershipControls = DeleteBucketOwnershipControlsResponse
  request  = delete s3
  response = receiveNull DeleteBucketOwnershipControlsResponse'

instance Hashable DeleteBucketOwnershipControls where

instance NFData DeleteBucketOwnershipControls where

instance ToHeaders DeleteBucketOwnershipControls
         where
  toHeaders DeleteBucketOwnershipControls' {..} =
    mconcat ["x-amz-expected-bucket-owner" =# _dbocExpectedBucketOwner]

instance ToPath DeleteBucketOwnershipControls where
  toPath DeleteBucketOwnershipControls' {..} = mconcat ["/", toBS _dbocBucket]
