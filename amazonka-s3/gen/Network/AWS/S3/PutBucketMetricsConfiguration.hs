...
data PutBucketMetricsConfiguration = PutBucketMetricsConfiguration'{_pbmcExpectedBucketOwner
                                                                    ::
                                                                    !(Maybe
                                                                        Text),
                                                                    _pbmcBucket
...
putBucketMetricsConfiguration pBucket_ pId_ pMetricsConfiguration_ =
  PutBucketMetricsConfiguration'
    { _pbmcExpectedBucketOwner  = Nothing
...
pbmcExpectedBucketOwner =
  lens _pbmcExpectedBucketOwner (\s a -> s { _pbmcExpectedBucketOwner = a })
...
pbmcMetricsConfiguration
  :: Lens' PutBucketMetricsConfiguration MetricsConfiguration
pbmcMetricsConfiguration =
  lens _pbmcMetricsConfiguration (\s a -> s { _pbmcMetricsConfiguration = a })

instance AWSRequest PutBucketMetricsConfiguration
         where
  type Rs PutBucketMetricsConfiguration = PutBucketMetricsConfigurationResponse
  request  = putXML s3
...
instance ToElement PutBucketMetricsConfiguration
         where
  toElement =
    mkElement "{http://s3.amazonaws.com/doc/2006-03-01/}MetricsConfiguration"
      . _pbmcMetricsConfiguration

instance ToHeaders PutBucketMetricsConfiguration
         where
  toHeaders PutBucketMetricsConfiguration' {..} =
    mconcat ["x-amz-expected-bucket-owner" =# _pbmcExpectedBucketOwner]

instance ToPath PutBucketMetricsConfiguration where
  toPath PutBucketMetricsConfiguration' {..} = mconcat ["/", toBS _pbmcBucket]
