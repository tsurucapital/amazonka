...
data GetBucketACL = GetBucketACL'{_gbaExpectedBucketOwner
                                  :: !(Maybe Text),
...
gbaExpectedBucketOwner =
  lens _gbaExpectedBucketOwner (\s a -> s { _gbaExpectedBucketOwner = a })
...
instance ToHeaders GetBucketACL where
  toHeaders GetBucketACL' {..} =
    mconcat ["x-amz-expected-bucket-owner" =# _gbaExpectedBucketOwner]
...
getBucketACLResponse
  ...
  -> GetBucketACLResponse
getBucketACLResponse pResponseStatus_ = GetBucketACLResponse'
