...
data DeletePublicAccessBlock = DeletePublicAccessBlock'{_dpabExpectedBucketOwner
                                                        :: !(Maybe Text),
                                                        _dpabBucket ::
                                                        !BucketName}
...
deletePublicAccessBlock
  ...
  -> DeletePublicAccessBlock
deletePublicAccessBlock pBucket_ = DeletePublicAccessBlock'
...
dpabExpectedBucketOwner =
  lens _dpabExpectedBucketOwner (\s a -> s { _dpabExpectedBucketOwner = a })
...
instance ToHeaders DeletePublicAccessBlock where
  toHeaders DeletePublicAccessBlock' {..} =
    mconcat ["x-amz-expected-bucket-owner" =# _dpabExpectedBucketOwner]

instance ToPath DeletePublicAccessBlock where
  toPath DeletePublicAccessBlock' {..} = mconcat ["/", toBS _dpabBucket]
