...
data ObjectLockConfiguration = ObjectLockConfiguration'{_olcObjectLockEnabled
                                                        ::
                                                        !(Maybe
                                                            ObjectLockEnabled),
                                                        _olcRule ::
...
olcObjectLockEnabled =
  lens _olcObjectLockEnabled (\s a -> s { _olcObjectLockEnabled = a })
...
instance FromXML ObjectLockConfiguration where
  parseXML x =
    ObjectLockConfiguration' <$> (x .@? "ObjectLockEnabled") <*> (x .@? "Rule")
...
instance ToXML ObjectLockConfiguration where
  toXML ObjectLockConfiguration' {..} =
    mconcat ["ObjectLockEnabled" @= _olcObjectLockEnabled, "Rule" @= _olcRule]
