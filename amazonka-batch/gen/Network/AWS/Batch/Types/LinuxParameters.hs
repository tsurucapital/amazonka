...
--
-- * 'lpSwappiness' - This allows you to tune a container's memory swappiness behavior. A @swappiness@ value of @0@ causes swapping not to happen unless absolutely necessary. A @swappiness@ value of @100@ causes pages to be swapped very aggressively. Accepted values are whole numbers between @0@ and @100@ . If the @swappiness@ parameter isn't specified, a default value of @60@ is used. If a value isn't specified for @maxSwap@ then this parameter is ignored. This parameter maps to the @--memory-swappiness@ option to <https://docs.docker.com/engine/reference/run/ docker run> .
--
...
linuxParameters = LinuxParameters' { _lpSharedMemorySize   = Nothing
...
lpSharedMemorySize =
  lens _lpSharedMemorySize (\s a -> s { _lpSharedMemorySize = a })
...
lpInitProcessEnabled =
  lens _lpInitProcessEnabled (\s a -> s { _lpInitProcessEnabled = a })
