...
data GetBucketEncryption = GetBucketEncryption'{_gbeExpectedBucketOwner
                                                :: !(Maybe Text),
...
getBucketEncryption
  ...
  -> GetBucketEncryption
getBucketEncryption pBucket_ = GetBucketEncryption'
...
gbeExpectedBucketOwner =
  lens _gbeExpectedBucketOwner (\s a -> s { _gbeExpectedBucketOwner = a })
...
instance AWSRequest GetBucketEncryption where
  ...
  response = receiveXML
    (\s h x ->
      GetBucketEncryptionResponse' <$> (parseXML x) <*> (pure (fromEnum s))
    )
...
instance ToHeaders GetBucketEncryption where
  toHeaders GetBucketEncryption' {..} =
    mconcat ["x-amz-expected-bucket-owner" =# _gbeExpectedBucketOwner]
...
getBucketEncryptionResponse
  ...
  -> GetBucketEncryptionResponse
getBucketEncryptionResponse pResponseStatus_ = GetBucketEncryptionResponse'
...
-- | Undocumented member.
gbersServerSideEncryptionConfiguration
  :: Lens' GetBucketEncryptionResponse (Maybe ServerSideEncryptionConfiguration)
gbersServerSideEncryptionConfiguration = lens
  _gbersServerSideEncryptionConfiguration
  (\s a -> s { _gbersServerSideEncryptionConfiguration = a })

-- | -- | The response status code.
gbersResponseStatus :: Lens' GetBucketEncryptionResponse Int
gbersResponseStatus =
  lens _gbersResponseStatus (\s a -> s { _gbersResponseStatus = a })
