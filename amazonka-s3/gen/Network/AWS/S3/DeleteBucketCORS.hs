...
data DeleteBucketCORS = DeleteBucketCORS'{_dbcExpectedBucketOwner
                                          :: !(Maybe Text),
...
dbcExpectedBucketOwner =
  lens _dbcExpectedBucketOwner (\s a -> s { _dbcExpectedBucketOwner = a })
...
instance ToHeaders DeleteBucketCORS where
  toHeaders DeleteBucketCORS' {..} =
    mconcat ["x-amz-expected-bucket-owner" =# _dbcExpectedBucketOwner]
