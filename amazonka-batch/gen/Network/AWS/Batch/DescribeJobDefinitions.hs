...
djdJobDefinitionName =
  lens _djdJobDefinitionName (\s a -> s { _djdJobDefinitionName = a })

-- | A list of up to 100 job definition names or full Amazon Resource Name (ARN) entries.
djdJobDefinitions :: Lens' DescribeJobDefinitions [Text]
djdJobDefinitions =
  lens _djdJobDefinitions (\s a -> s { _djdJobDefinitions = a })
    . _Default
    . _Coerce
...
instance AWSPager DescribeJobDefinitions where
  page rq rs | stop (rs ^. djdrsNextToken) = Nothing
...
instance ToJSON DescribeJobDefinitions where
  toJSON DescribeJobDefinitions' {..} = object
    (catMaybes
      [ ("status" .=) <$> _djdStatus
...
describeJobDefinitionsResponse pResponseStatus_ =
  DescribeJobDefinitionsResponse' { _djdrsJobDefinitions = Nothing
...
djdrsJobDefinitions =
  lens _djdrsJobDefinitions (\s a -> s { _djdrsJobDefinitions = a })
    . _Default
    . _Coerce
...
djdrsResponseStatus =
  lens _djdrsResponseStatus (\s a -> s { _djdrsResponseStatus = a })
