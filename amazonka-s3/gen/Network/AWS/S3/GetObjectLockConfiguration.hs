...
data GetObjectLockConfiguration = GetObjectLockConfiguration'{_golcExpectedBucketOwner
                                                              :: !(Maybe Text),
                                                              _golcBucket ::
                                                              !BucketName}
...
getObjectLockConfiguration
  ...
  -> GetObjectLockConfiguration
getObjectLockConfiguration pBucket_ = GetObjectLockConfiguration'
...
golcExpectedBucketOwner =
  lens _golcExpectedBucketOwner (\s a -> s { _golcExpectedBucketOwner = a })
...
instance AWSRequest GetObjectLockConfiguration where
  type Rs GetObjectLockConfiguration = GetObjectLockConfigurationResponse
  request  = get s3
  response = receiveXML
    (\s h x ->
      GetObjectLockConfigurationResponse'
...
instance ToHeaders GetObjectLockConfiguration where
  toHeaders GetObjectLockConfiguration' {..} =
    mconcat ["x-amz-expected-bucket-owner" =# _golcExpectedBucketOwner]

instance ToPath GetObjectLockConfiguration where
  toPath GetObjectLockConfiguration' {..} = mconcat ["/", toBS _golcBucket]
...
data GetObjectLockConfigurationResponse = GetObjectLockConfigurationResponse'{_golcrsObjectLockConfiguration
                                                                              ::
                                                                              !(Maybe
...
getObjectLockConfigurationResponse pResponseStatus_ =
  GetObjectLockConfigurationResponse' { _golcrsObjectLockConfiguration = Nothing
                                      , _golcrsResponseStatus = pResponseStatus_
                                      }

-- | The specified bucket's Object Lock configuration.
golcrsObjectLockConfiguration
  :: Lens' GetObjectLockConfigurationResponse (Maybe ObjectLockConfiguration)
golcrsObjectLockConfiguration = lens
  _golcrsObjectLockConfiguration
  (\s a -> s { _golcrsObjectLockConfiguration = a })

-- | -- | The response status code.
golcrsResponseStatus :: Lens' GetObjectLockConfigurationResponse Int
golcrsResponseStatus =
  lens _golcrsResponseStatus (\s a -> s { _golcrsResponseStatus = a })
