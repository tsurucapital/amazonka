...
data GetBucketPolicyStatus = GetBucketPolicyStatus'{_gbpsExpectedBucketOwner
                                                    :: !(Maybe Text),
...
getBucketPolicyStatus
  ...
  -> GetBucketPolicyStatus
getBucketPolicyStatus pBucket_ = GetBucketPolicyStatus'
...
gbpsExpectedBucketOwner =
  lens _gbpsExpectedBucketOwner (\s a -> s { _gbpsExpectedBucketOwner = a })
...
instance AWSRequest GetBucketPolicyStatus where
  ...
  response = receiveXML
    (\s h x ->
      GetBucketPolicyStatusResponse' <$> (parseXML x) <*> (pure (fromEnum s))
    )
...
instance ToHeaders GetBucketPolicyStatus where
  toHeaders GetBucketPolicyStatus' {..} =
    mconcat ["x-amz-expected-bucket-owner" =# _gbpsExpectedBucketOwner]
...
data GetBucketPolicyStatusResponse = GetBucketPolicyStatusResponse'{_gbpsrsPolicyStatus
                                                                    ::
                                                                    !(Maybe
                                                                        PolicyStatus),
                                                                    _gbpsrsResponseStatus
                                                                    :: !Int}
                                       deriving (Eq, Read, Show, Data, Typeable,
                                                 Generic)
...
getBucketPolicyStatusResponse
  ...
  -> GetBucketPolicyStatusResponse
getBucketPolicyStatusResponse pResponseStatus_ = GetBucketPolicyStatusResponse'
...
gbpsrsPolicyStatus =
  lens _gbpsrsPolicyStatus (\s a -> s { _gbpsrsPolicyStatus = a })

-- | -- | The response status code.
gbpsrsResponseStatus :: Lens' GetBucketPolicyStatusResponse Int
gbpsrsResponseStatus =
  lens _gbpsrsResponseStatus (\s a -> s { _gbpsrsResponseStatus = a })
