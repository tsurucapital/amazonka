...
dceComputeEnvironment =
  lens _dceComputeEnvironment (\s a -> s { _dceComputeEnvironment = a })
...
instance AWSRequest DeleteComputeEnvironment where
  ...
  response = receiveEmpty
    (\s h x -> DeleteComputeEnvironmentResponse' <$> (pure (fromEnum s)))
...
instance ToJSON DeleteComputeEnvironment where
  toJSON DeleteComputeEnvironment' {..} =
    object (catMaybes [Just ("computeEnvironment" .= _dceComputeEnvironment)])
...
dcersResponseStatus =
  lens _dcersResponseStatus (\s a -> s { _dcersResponseStatus = a })
