...
selectObjectContent pBucket_ pKey_ pExpression_ pExpressionType_ pInputSerialization_ pOutputSerialization_
  = SelectObjectContent' { _socSSECustomerAlgorithm = Nothing
...
socExpectedBucketOwner =
  lens _socExpectedBucketOwner (\s a -> s { _socExpectedBucketOwner = a })
...
socExpressionType =
  lens _socExpressionType (\s a -> s { _socExpressionType = a })
...
instance ToHeaders SelectObjectContent where
  toHeaders SelectObjectContent' {..} = mconcat
...
instance ToXML SelectObjectContent where
  toXML SelectObjectContent' {..} = mconcat
...
selectObjectContentResponse
  ...
  -> SelectObjectContentResponse
selectObjectContentResponse pResponseStatus_ = SelectObjectContentResponse'
