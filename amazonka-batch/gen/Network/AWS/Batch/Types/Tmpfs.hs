...
data Tmpfs = Tmpfs'{_tMountOptions ::
                    !(Maybe [Text]),
                    _tContainerPath :: !Text, _tSize :: !Int}
...
tmpfs
  ...
  -> Int -- ^ 'tSize'
  -> Tmpfs
tmpfs pContainerPath_ pSize_ = Tmpfs' { _tMountOptions  = Nothing
...
tMountOptions =
  lens _tMountOptions (\s a -> s { _tMountOptions = a }) . _Default . _Coerce
