...
--
--
-- /Important:/ Jobs run on Fargate resources don't run for more than 14 days. After 14 days, the Fargate resources might no longer be available and the job is terminated.
--
module Network.AWS.Batch.SubmitJob
  (
    -- * Creating a Request
...
--
-- * 'sjPropagateTags' - Specifies whether to propagate the tags from the job or job definition to the corresponding Amazon ECS task. If no value is specified, the tags aren't propagated. Tags can only be propagated to the tasks during task creation. For tags with the same name, job tags are given priority over job definitions tags. If the total number of combined tags from the job and job definition is over 50, the job is moved to the @FAILED@ state. When specified, this overrides the tag propagation setting in the job definition.
--
...
submitJob
  ...
  -> SubmitJob
submitJob pJobName_ pJobQueue_ pJobDefinition_ = SubmitJob'
  { _sjNodeOverrides      = Nothing
  , _sjPropagateTags      = Nothing
...
sjPropagateTags :: Lens' SubmitJob (Maybe Bool)
sjPropagateTags = lens _sjPropagateTags (\s a -> s { _sjPropagateTags = a })

-- | A list of container overrides in JSON format that specify the name of a container in the specified job definition and the overrides it should receive. You can override the default command for a container (that is specified in the job definition or the Docker image) with a @command@ override. You can also override existing environment variables (that are specified in the job definition or Docker image) on a container or add new environment variables to it with an @environment@ override.
sjContainerOverrides :: Lens' SubmitJob (Maybe ContainerOverrides)
sjContainerOverrides =
  lens _sjContainerOverrides (\s a -> s { _sjContainerOverrides = a })
...
sjDependsOn =
  lens _sjDependsOn (\s a -> s { _sjDependsOn = a }) . _Default . _Coerce

-- | Additional parameters passed to the job that replace parameter substitution placeholders that are set in the job definition. Parameters are specified as a key and value pair mapping. Parameters in a @SubmitJob@ request override any corresponding parameter defaults from the job definition.
sjParameters :: Lens' SubmitJob (HashMap Text Text)
sjParameters =
  lens _sjParameters (\s a -> s { _sjParameters = a }) . _Default . _Map
...
sjArrayProperties =
  lens _sjArrayProperties (\s a -> s { _sjArrayProperties = a })
...
instance ToJSON SubmitJob where
  toJSON SubmitJob' {..} = object
    (catMaybes
      [ ("nodeOverrides" .=) <$> _sjNodeOverrides
      , ("propagateTags" .=) <$> _sjPropagateTags
...
data SubmitJobResponse = SubmitJobResponse'{_sjrsJobARN
                                            :: !(Maybe Text),
...
submitJobResponse
  ...
  -> Text -- ^ 'sjrsJobId'
  -> SubmitJobResponse
submitJobResponse pResponseStatus_ pJobName_ pJobId_ = SubmitJobResponse'
  { _sjrsJobARN         = Nothing
...
sjrsResponseStatus =
  lens _sjrsResponseStatus (\s a -> s { _sjrsResponseStatus = a })
