...
createBucket
  :: BucketName -- ^ 'cbBucket'
  -> CreateBucket
createBucket pBucket_ = CreateBucket' { _cbGrantReadACP               = Nothing
...
cbObjectLockEnabledForBucket = lens
  _cbObjectLockEnabledForBucket
  (\s a -> s { _cbObjectLockEnabledForBucket = a })
...
cbCreateBucketConfiguration
  :: Lens' CreateBucket (Maybe CreateBucketConfiguration)
cbCreateBucketConfiguration = lens
  _cbCreateBucketConfiguration
...
instance AWSRequest CreateBucket where
  ...
  response = receiveEmpty
    (\s h x ->
      CreateBucketResponse' <$> (h .#? "Location") <*> (pure (fromEnum s))
    )
...
instance ToHeaders CreateBucket where
  toHeaders CreateBucket' {..} = mconcat
    ...
    , "x-amz-bucket-object-lock-enabled" =# _cbObjectLockEnabledForBucket
...
data CreateBucketResponse = CreateBucketResponse'{_cbrsLocation
                                                  :: !(Maybe Text),
...
createBucketResponse
  ...
  -> CreateBucketResponse
createBucketResponse pResponseStatus_ = CreateBucketResponse'
