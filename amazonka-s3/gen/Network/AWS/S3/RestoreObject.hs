...
restoreObject
  ...
  -> ObjectKey -- ^ 'roKey'
  -> RestoreObject
restoreObject pBucket_ pKey_ = RestoreObject'
...
roExpectedBucketOwner =
  lens _roExpectedBucketOwner (\s a -> s { _roExpectedBucketOwner = a })
...
instance ToElement RestoreObject where
  toElement =
    mkElement "{http://s3.amazonaws.com/doc/2006-03-01/}RestoreRequest"
      . _roRestoreRequest
...
instance ToPath RestoreObject where
  toPath RestoreObject' {..} = mconcat ["/", toBS _roBucket, "/", toBS _roKey]
...
restoreObjectResponse
  ...
  -> RestoreObjectResponse
restoreObjectResponse pResponseStatus_ = RestoreObjectResponse'
...
rorsRequestCharged =
  lens _rorsRequestCharged (\s a -> s { _rorsRequestCharged = a })

-- | Indicates the path in the provided S3 output location where Select results will be restored to.
rorsRestoreOutputPath :: Lens' RestoreObjectResponse (Maybe Text)
rorsRestoreOutputPath =
  lens _rorsRestoreOutputPath (\s a -> s { _rorsRestoreOutputPath = a })

-- | -- | The response status code.
rorsResponseStatus :: Lens' RestoreObjectResponse Int
rorsResponseStatus =
  lens _rorsResponseStatus (\s a -> s { _rorsResponseStatus = a })
