...
data NodePropertyOverride = NodePropertyOverride'{_npoContainerOverrides
                                                  ::
...
--
-- * 'npoTargetNodes' - The range of nodes, using node index values, that's used to override. A range of @0:3@ indicates nodes with index values of @0@ through @3@ . If the starting range value is omitted (@:n@ ), then @0@ is used to start the range. If the ending range value is omitted (@n:@ ), then the highest possible node index is used to end the range.
nodePropertyOverride
  :: Text -- ^ 'npoTargetNodes'
  -> NodePropertyOverride
nodePropertyOverride pTargetNodes_ = NodePropertyOverride'
...
npoContainerOverrides =
  lens _npoContainerOverrides (\s a -> s { _npoContainerOverrides = a })
...
instance ToJSON NodePropertyOverride where
  toJSON NodePropertyOverride' {..} = object
    (catMaybes
