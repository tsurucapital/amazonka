...
data GetObjectLegalHold = GetObjectLegalHold'{_golhVersionId
...
getObjectLegalHold
  ...
  -> ObjectKey -- ^ 'golhKey'
  -> GetObjectLegalHold
getObjectLegalHold pBucket_ pKey_ = GetObjectLegalHold'
...
golhExpectedBucketOwner =
  lens _golhExpectedBucketOwner (\s a -> s { _golhExpectedBucketOwner = a })
...
instance AWSRequest GetObjectLegalHold where
  ...
  response = receiveXML
    (\s h x ->
      GetObjectLegalHoldResponse' <$> (parseXML x) <*> (pure (fromEnum s))
    )
...
instance ToHeaders GetObjectLegalHold where
  toHeaders GetObjectLegalHold' {..} = mconcat
...
data GetObjectLegalHoldResponse = GetObjectLegalHoldResponse'{_golhrsLegalHold
                                                              ::
                                                              !(Maybe
                                                                  ObjectLockLegalHold),
                                                              _golhrsResponseStatus
                                                              :: !Int}
...
getObjectLegalHoldResponse
  ...
  -> GetObjectLegalHoldResponse
getObjectLegalHoldResponse pResponseStatus_ = GetObjectLegalHoldResponse'
...
golhrsResponseStatus =
  lens _golhrsResponseStatus (\s a -> s { _golhrsResponseStatus = a })
