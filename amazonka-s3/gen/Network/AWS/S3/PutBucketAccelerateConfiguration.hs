...
data PutBucketAccelerateConfiguration = PutBucketAccelerateConfiguration'{_pbacExpectedBucketOwner
                                                                          ::
                                                                          !(Maybe
                                                                              Text),
                                                                          _pbacBucket
...
pbacExpectedBucketOwner =
  lens _pbacExpectedBucketOwner (\s a -> s { _pbacExpectedBucketOwner = a })
...
pbacAccelerateConfiguration
  :: Lens' PutBucketAccelerateConfiguration AccelerateConfiguration
pbacAccelerateConfiguration = lens
  _pbacAccelerateConfiguration
  (\s a -> s { _pbacAccelerateConfiguration = a })
...
instance ToElement PutBucketAccelerateConfiguration
         where
  toElement =
    mkElement "{http://s3.amazonaws.com/doc/2006-03-01/}AccelerateConfiguration"
      . _pbacAccelerateConfiguration

instance ToHeaders PutBucketAccelerateConfiguration
         where
  toHeaders PutBucketAccelerateConfiguration' {..} =
    mconcat ["x-amz-expected-bucket-owner" =# _pbacExpectedBucketOwner]
