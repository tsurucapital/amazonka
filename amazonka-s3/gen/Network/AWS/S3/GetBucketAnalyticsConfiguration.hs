...
data GetBucketAnalyticsConfiguration = GetBucketAnalyticsConfiguration'{_getExpectedBucketOwner
                                                                        ::
                                                                        !(Maybe
                                                                            Text),
                                                                        _getBucket
...
getBucketAnalyticsConfiguration pBucket_ pId_ =
  GetBucketAnalyticsConfiguration' { _getExpectedBucketOwner = Nothing
                                   , _getBucket              = pBucket_
                                   , _getId                  = pId_
                                   }
...
getExpectedBucketOwner =
  lens _getExpectedBucketOwner (\s a -> s { _getExpectedBucketOwner = a })
...
instance ToHeaders GetBucketAnalyticsConfiguration
  ...
  toHeaders GetBucketAnalyticsConfiguration' {..} =
    mconcat ["x-amz-expected-bucket-owner" =# _getExpectedBucketOwner]

instance ToPath GetBucketAnalyticsConfiguration where
  toPath GetBucketAnalyticsConfiguration' {..} = mconcat ["/", toBS _getBucket]
...
gbacrsAnalyticsConfiguration = lens
  _gbacrsAnalyticsConfiguration
  (\s a -> s { _gbacrsAnalyticsConfiguration = a })

-- | -- | The response status code.
gbacrsResponseStatus :: Lens' GetBucketAnalyticsConfigurationResponse Int
gbacrsResponseStatus =
  lens _gbacrsResponseStatus (\s a -> s { _gbacrsResponseStatus = a })
