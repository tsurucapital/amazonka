...
data EC2Configuration = EC2Configuration'{_ecImageIdOverride
                                          :: !(Maybe Text),
                                          _ecImageType :: !Text}
...
instance FromJSON EC2Configuration where
  parseJSON = withObject
    "EC2Configuration"
    (\x ->
      EC2Configuration' <$> (x .:? "imageIdOverride") <*> (x .: "imageType")
    )

instance Hashable EC2Configuration where

instance NFData EC2Configuration where

instance ToJSON EC2Configuration where
  toJSON EC2Configuration' {..} = object
    (catMaybes
