...
gbtExpectedBucketOwner =
  lens _gbtExpectedBucketOwner (\s a -> s { _gbtExpectedBucketOwner = a })
...
instance ToHeaders GetBucketTagging where
  toHeaders GetBucketTagging' {..} =
    mconcat ["x-amz-expected-bucket-owner" =# _gbtExpectedBucketOwner]
...
getBucketTaggingResponse
  ...
  -> GetBucketTaggingResponse
getBucketTaggingResponse pResponseStatus_ = GetBucketTaggingResponse'
