...
data DeleteBucketReplication = DeleteBucketReplication'{_dbrExpectedBucketOwner
                                                        :: !(Maybe Text),
                                                        _dbrBucket ::
                                                        !BucketName}
...
deleteBucketReplication
  ...
  -> DeleteBucketReplication
deleteBucketReplication pBucket_ = DeleteBucketReplication'
...
dbrExpectedBucketOwner =
  lens _dbrExpectedBucketOwner (\s a -> s { _dbrExpectedBucketOwner = a })
...
instance ToHeaders DeleteBucketReplication where
  toHeaders DeleteBucketReplication' {..} =
    mconcat ["x-amz-expected-bucket-owner" =# _dbrExpectedBucketOwner]

instance ToPath DeleteBucketReplication where
  toPath DeleteBucketReplication' {..} = mconcat ["/", toBS _dbrBucket]
