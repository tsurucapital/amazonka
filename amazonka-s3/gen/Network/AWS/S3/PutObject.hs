...
data PutObject = PutObject'{_poContentLength ::
                            !(Maybe Integer),
                            ...
                            _poBucket :: !BucketName, _poKey :: !ObjectKey,
                            _poBody :: !RqBody}
...
putObject
  ...
  -> RqBody -- ^ 'poBody'
  -> PutObject
putObject pBucket_ pKey_ pBody_ = PutObject'
...
poBucketKeyEnabled =
  lens _poBucketKeyEnabled (\s a -> s { _poBucketKeyEnabled = a })

-- | If the bucket is configured as a website, redirects requests for this object to another object in the same bucket or to an external URL. Amazon S3 stores the value of this header in the object metadata. For information about object metadata, see <https://docs.aws.amazon.com/AmazonS3/latest/dev/UsingMetadata.html Object Key and Metadata> . In the following example, the request header sets the redirect to an object (anotherPage.html) in the same bucket: @x-amz-website-redirect-location: /anotherPage.html@  In the following example, the request header sets the object redirect to another website: @x-amz-website-redirect-location: http://www.example.com/@  For more information about website hosting in Amazon S3, see <https://docs.aws.amazon.com/AmazonS3/latest/dev/WebsiteHosting.html Hosting Websites on Amazon S3> and <https://docs.aws.amazon.com/AmazonS3/latest/dev/how-to-page-redirect.html How to Configure Website Page Redirects> .
poWebsiteRedirectLocation :: Lens' PutObject (Maybe Text)
poWebsiteRedirectLocation =
  lens _poWebsiteRedirectLocation (\s a -> s { _poWebsiteRedirectLocation = a })
...
poSSEKMSKeyId =
  lens _poSSEKMSKeyId (\s a -> s { _poSSEKMSKeyId = a }) . mapping _Sensitive
...
poObjectLockRetainUntilDate =
  lens _poObjectLockRetainUntilDate
       (\s a -> s { _poObjectLockRetainUntilDate = a })
    . mapping _Time
...
poSSEKMSEncryptionContext =
  lens _poSSEKMSEncryptionContext (\s a -> s { _poSSEKMSEncryptionContext = a })
    . mapping _Sensitive
...
poObjectLockLegalHoldStatus = lens
  _poObjectLockLegalHoldStatus
  (\s a -> s { _poObjectLockLegalHoldStatus = a })
...
poExpectedBucketOwner =
  lens _poExpectedBucketOwner (\s a -> s { _poExpectedBucketOwner = a })
...
instance ToHeaders PutObject where
  toHeaders PutObject' {..} = mconcat
    ...
    , "x-amz-server-side-encryption-bucket-key-enabled" =# _poBucketKeyEnabled
    ...
    , "x-amz-object-lock-retain-until-date" =# _poObjectLockRetainUntilDate
    , "x-amz-meta-" =# _poMetadata
    , "x-amz-server-side-encryption-context" =# _poSSEKMSEncryptionContext
...
instance ToPath PutObject where
  toPath PutObject' {..} = mconcat ["/", toBS _poBucket, "/", toBS _poKey]

instance ToQuery PutObject where
  toQuery = const mempty

-- | /See:/ 'putObjectResponse' smart constructor.
data PutObjectResponse = PutObjectResponse'{_porsRequestCharged
                                            :: !(Maybe RequestCharged),
                                            _porsETag :: !(Maybe ETag),
                                            _porsVersionId ::
                                            !(Maybe ObjectVersionId),
                                            _porsExpiration :: !(Maybe Text),
                                            _porsSSECustomerAlgorithm ::
                                            !(Maybe Text),
                                            _porsBucketKeyEnabled ::
                                            !(Maybe Bool),
                                            _porsSSECustomerKeyMD5 ::
                                            !(Maybe Text),
                                            _porsSSEKMSKeyId ::
                                            !(Maybe (Sensitive Text)),
...
--
-- * 'porsBucketKeyEnabled' - Indicates whether the uploaded object uses an S3 Bucket Key for server-side encryption with AWS KMS (SSE-KMS).
--
...
porsRequestCharged =
  lens _porsRequestCharged (\s a -> s { _porsRequestCharged = a })
...
porsSSECustomerAlgorithm =
  lens _porsSSECustomerAlgorithm (\s a -> s { _porsSSECustomerAlgorithm = a })

-- | Indicates whether the uploaded object uses an S3 Bucket Key for server-side encryption with AWS KMS (SSE-KMS).
porsBucketKeyEnabled :: Lens' PutObjectResponse (Maybe Bool)
porsBucketKeyEnabled =
  lens _porsBucketKeyEnabled (\s a -> s { _porsBucketKeyEnabled = a })
...
porsSSEKMSKeyId = lens _porsSSEKMSKeyId (\s a -> s { _porsSSEKMSKeyId = a })
  . mapping _Sensitive

-- | If present, specifies the AWS KMS Encryption Context to use for object encryption. The value of this header is a base64-encoded UTF-8 string holding JSON with the encryption context key-value pairs.
porsSSEKMSEncryptionContext :: Lens' PutObjectResponse (Maybe Text)
porsSSEKMSEncryptionContext =
  lens _porsSSEKMSEncryptionContext
       (\s a -> s { _porsSSEKMSEncryptionContext = a })
    . mapping _Sensitive
...
porsServerSideEncryption =
  lens _porsServerSideEncryption (\s a -> s { _porsServerSideEncryption = a })

-- | -- | The response status code.
porsResponseStatus :: Lens' PutObjectResponse Int
porsResponseStatus =
  lens _porsResponseStatus (\s a -> s { _porsResponseStatus = a })
