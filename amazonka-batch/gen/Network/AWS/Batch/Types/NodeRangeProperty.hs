...
data NodeRangeProperty = NodeRangeProperty'{_nrpContainer
...
nodeRangeProperty
  :: Text -- ^ 'nrpTargetNodes'
  -> NodeRangeProperty
nodeRangeProperty pTargetNodes_ = NodeRangeProperty'
...
instance FromJSON NodeRangeProperty where
  parseJSON = withObject
    "NodeRangeProperty"
    (\x -> NodeRangeProperty' <$> (x .:? "container") <*> (x .: "targetNodes"))
...
instance ToJSON NodeRangeProperty where
  toJSON NodeRangeProperty' {..} = object
    (catMaybes
