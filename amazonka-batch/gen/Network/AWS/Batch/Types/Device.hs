...
data Device = Device'{_dContainerPath ::
                      !(Maybe Text),
...
--
-- * 'dContainerPath' - The path inside the container used to expose the host device. By default the @hostPath@ value is used.
--
...
device
  :: Text -- ^ 'dHostPath'
  -> Device
device pHostPath_ = Device' { _dContainerPath = Nothing
...
dPermissions =
  lens _dPermissions (\s a -> s { _dPermissions = a }) . _Default . _Coerce
