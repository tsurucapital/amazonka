...
getObject
  ...
  -> ObjectKey -- ^ 'goKey'
  -> GetObject
getObject pBucket_ pKey_ = GetObject' { _goIfMatch                    = Nothing
...
goResponseContentDisposition = lens
...
goResponseContentLanguage =
  lens _goResponseContentLanguage (\s a -> s { _goResponseContentLanguage = a })
...
goResponseContentEncoding =
  lens _goResponseContentEncoding (\s a -> s { _goResponseContentEncoding = a })

-- | Return the object only if it has been modified since the specified time, otherwise return a 304 (not modified).
goIfModifiedSince :: Lens' GetObject (Maybe UTCTime)
goIfModifiedSince =
  lens _goIfModifiedSince (\s a -> s { _goIfModifiedSince = a }) . mapping _Time
...
goResponseExpires =
  lens _goResponseExpires (\s a -> s { _goResponseExpires = a }) . mapping _Time
...
goExpectedBucketOwner =
  lens _goExpectedBucketOwner (\s a -> s { _goExpectedBucketOwner = a })
...
instance ToPath GetObject where
  toPath GetObject' {..} = mconcat ["/", toBS _goBucket, "/", toBS _goKey]
...
--
-- * 'gorsBucketKeyEnabled' - Indicates whether the object uses an S3 Bucket Key for server-side encryption with AWS KMS (SSE-KMS).
--
...
getObjectResponse
  ...
  -> RsBody -- ^ 'gorsBody'
  -> GetObjectResponse
getObjectResponse pResponseStatus_ pBody_ = GetObjectResponse'
...
gorsRequestCharged =
  lens _gorsRequestCharged (\s a -> s { _gorsRequestCharged = a })
...
gorsObjectLockMode =
  lens _gorsObjectLockMode (\s a -> s { _gorsObjectLockMode = a })

-- | The date and time at which the object is no longer cacheable.
gorsExpires :: Lens' GetObjectResponse (Maybe UTCTime)
gorsExpires =
  lens _gorsExpires (\s a -> s { _gorsExpires = a }) . mapping _Time
...
gorsSSECustomerAlgorithm =
  lens _gorsSSECustomerAlgorithm (\s a -> s { _gorsSSECustomerAlgorithm = a })
...
gorsBucketKeyEnabled :: Lens' GetObjectResponse (Maybe Bool)
gorsBucketKeyEnabled =
  lens _gorsBucketKeyEnabled (\s a -> s { _gorsBucketKeyEnabled = a })

-- | If the bucket is configured as a website, redirects requests for this object to another object in the same bucket or to an external URL. Amazon S3 stores the value of this header in the object metadata.
gorsWebsiteRedirectLocation :: Lens' GetObjectResponse (Maybe Text)
gorsWebsiteRedirectLocation = lens
  _gorsWebsiteRedirectLocation
...
gorsSSEKMSKeyId = lens _gorsSSEKMSKeyId (\s a -> s { _gorsSSEKMSKeyId = a })
  . mapping _Sensitive
...
gorsObjectLockRetainUntilDate =
  lens _gorsObjectLockRetainUntilDate
       (\s a -> s { _gorsObjectLockRetainUntilDate = a })
    . mapping _Time
...
gorsReplicationStatus =
  lens _gorsReplicationStatus (\s a -> s { _gorsReplicationStatus = a })
...
gorsLastModified =
  lens _gorsLastModified (\s a -> s { _gorsLastModified = a }) . mapping _Time

-- | Indicates whether this object has an active legal hold. This field is only returned if you have permission to view an object's legal hold status.
gorsObjectLockLegalHoldStatus
  :: Lens' GetObjectResponse (Maybe ObjectLockLegalHoldStatus)
gorsObjectLockLegalHoldStatus = lens
  _gorsObjectLockLegalHoldStatus
  (\s a -> s { _gorsObjectLockLegalHoldStatus = a })
...
gorsServerSideEncryption =
  lens _gorsServerSideEncryption (\s a -> s { _gorsServerSideEncryption = a })
