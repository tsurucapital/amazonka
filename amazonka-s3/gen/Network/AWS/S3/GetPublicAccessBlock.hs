...
data GetPublicAccessBlock = GetPublicAccessBlock'{_gpabExpectedBucketOwner
                                                  :: !(Maybe Text),
...
getPublicAccessBlock
  ...
  -> GetPublicAccessBlock
getPublicAccessBlock pBucket_ = GetPublicAccessBlock'
...
gpabExpectedBucketOwner =
  lens _gpabExpectedBucketOwner (\s a -> s { _gpabExpectedBucketOwner = a })
...
instance AWSRequest GetPublicAccessBlock where
  ...
  response = receiveXML
    (\s h x ->
      GetPublicAccessBlockResponse' <$> (parseXML x) <*> (pure (fromEnum s))
    )
...
instance ToHeaders GetPublicAccessBlock where
  toHeaders GetPublicAccessBlock' {..} =
    mconcat ["x-amz-expected-bucket-owner" =# _gpabExpectedBucketOwner]
...
data GetPublicAccessBlockResponse = GetPublicAccessBlockResponse'{_gpabrsPublicAccessBlockConfiguration
                                                                  ::
                                                                  !(Maybe
                                                                      PublicAccessBlockConfiguration),
                                                                  _gpabrsResponseStatus
                                                                  :: !Int}
...
getPublicAccessBlockResponse
  ...
  -> GetPublicAccessBlockResponse
getPublicAccessBlockResponse pResponseStatus_ = GetPublicAccessBlockResponse'
...
gpabrsPublicAccessBlockConfiguration
  :: Lens' GetPublicAccessBlockResponse (Maybe PublicAccessBlockConfiguration)
gpabrsPublicAccessBlockConfiguration = lens
  _gpabrsPublicAccessBlockConfiguration
  (\s a -> s { _gpabrsPublicAccessBlockConfiguration = a })

-- | -- | The response status code.
gpabrsResponseStatus :: Lens' GetPublicAccessBlockResponse Int
gpabrsResponseStatus =
  lens _gpabrsResponseStatus (\s a -> s { _gpabrsResponseStatus = a })
