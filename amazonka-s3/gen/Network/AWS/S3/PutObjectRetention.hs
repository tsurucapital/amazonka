...
data PutObjectRetention = PutObjectRetention'{_porRetention
...
putObjectRetention
  ...
  -> ObjectKey -- ^ 'porKey'
  -> PutObjectRetention
putObjectRetention pBucket_ pKey_ = PutObjectRetention'
...
porBypassGovernanceRetention = lens
  _porBypassGovernanceRetention
  (\s a -> s { _porBypassGovernanceRetention = a })

-- | The account id of the expected bucket owner. If the bucket is owned by a different account, the request will fail with an HTTP @403 (Access Denied)@ error.
porExpectedBucketOwner :: Lens' PutObjectRetention (Maybe Text)
porExpectedBucketOwner =
  lens _porExpectedBucketOwner (\s a -> s { _porExpectedBucketOwner = a })
...
instance ToHeaders PutObjectRetention where
  toHeaders PutObjectRetention' {..} = mconcat
    [ "x-amz-request-payer" =# _porRequestPayer
    , "Content-MD5" =# _porContentMD5
    , "x-amz-bypass-governance-retention" =# _porBypassGovernanceRetention
...
data PutObjectRetentionResponse = PutObjectRetentionResponse'{_porrsRequestCharged
                                                              ::
                                                              !(Maybe
                                                                  RequestCharged),
...
putObjectRetentionResponse
  ...
  -> PutObjectRetentionResponse
putObjectRetentionResponse pResponseStatus_ = PutObjectRetentionResponse'
...
porrsRequestCharged =
  lens _porrsRequestCharged (\s a -> s { _porrsRequestCharged = a })

-- | -- | The response status code.
porrsResponseStatus :: Lens' PutObjectRetentionResponse Int
porrsResponseStatus =
  lens _porrsResponseStatus (\s a -> s { _porrsResponseStatus = a })
