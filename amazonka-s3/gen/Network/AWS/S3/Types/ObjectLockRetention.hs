...
data ObjectLockRetention = ObjectLockRetention'{_olrMode
                                                ::
                                                !(Maybe
...
olrRetainUntilDate =
  lens _olrRetainUntilDate (\s a -> s { _olrRetainUntilDate = a })
    . mapping _Time

instance FromXML ObjectLockRetention where
  parseXML x =
    ObjectLockRetention' <$> (x .@? "Mode") <*> (x .@? "RetainUntilDate")
...
instance ToXML ObjectLockRetention where
  toXML ObjectLockRetention' {..} =
    mconcat ["Mode" @= _olrMode, "RetainUntilDate" @= _olrRetainUntilDate]
